...
|The audience to use for the client assertion. If not set this defaults to the IDP's token endpoint URL.

|Send Client ID with token request
|Send the Client ID with the token request when using JWT assertion as client authentication. The Client ID is optional when using JWT assertion but some providers require it to be sent.
